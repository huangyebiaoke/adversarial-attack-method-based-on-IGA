...
def muteChild(child):
    ...
    for i in range(temp_child.size):
        if(random.random()<_mutate_rate):
            temp_child[i] = 0. if temp_child[i]>.5 else 1.
    return temp_child
...
def muteChild2(child):
    ...
    if(random.random()<_mutate_rate):
        ...
        temp_child[index]= 0. if temp_child[index]>.5 else 1.
    return temp_child
...
def getPopulation():
    ...
    for i in range(t_test.shape[0]):
        if np.argmax(t_test[i:i+1])==np.argmax(test_label):
...
if __name__=='__main__':
    ...
    for iter in range(iteration):
        ...
        for i in range(population_size):
            ...
            # print('iteration:',iter,'confidence:',i,confidence_list[i])
